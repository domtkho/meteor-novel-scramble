...
Scripts.allow({
  update: function(userId, script){ return true; },
  remove: function(userId, script){ return ownsDocument(userId, script); }    // allows user to remove scripts as long as they own the script
});                                                                           //  (see ownsDocument function defined in permissions.js)
...
Scripts.deny({                                                                           // Denies user from updating if...
  update: function(userId, script, fieldNames){                                          // (fieldNames is an array of fields being modified)
    return ( _.without(fieldNames, 'title', 'genre', 'content', 'writers').length > 0 ); // ...the sub-array that excludes the allowed fields > 0
